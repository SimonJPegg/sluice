...
class InMemorySlidingWindowCounterTest {
  ...
  @Test
  fun `window rolls over, previous count carries forward with weight`() = runTest {
    val clock = FakeClock()
    val algorithm = InMemorySlidingWindowCounter(clock)
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
  ...
  @Test
  fun `two windows stale, previous count discarded entirely, fresh start`() = runTest {
    val clock = FakeClock()
    val algorithm = InMemorySlidingWindowCounter(clock)
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
  ...
  @Test
  fun `burst at window boundary, previous window's weight prevents 2x burst`() = runTest {
    val clock = FakeClock()
    val algorithm = InMemorySlidingWindowCounter(clock)
    val key = "test-key"
    // start the clock
