...
class RedisSlidingWindowLogTest: RedisTest() {
  ...
  @Test fun `first request returns Allowed with remaining = limit - 1`() = runBlocking {
  ...
  @Test fun `multiple requests within same window, remaining decreases`() = runBlocking {
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
  ...
  @Test fun `at limit, next request returns Denied`() = runBlocking {
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
      val result = algorithm.calculate(key, defaultPolicy)
  ...
  @Test fun `denied request returns retryAfter as time until oldest entry expires`() = runBlocking {
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
      val result = algorithm.calculate(key, defaultPolicy)
  ...
  @Test fun `requests outside window are pruned, counter resets after window passes`() = runBlocking {
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
      val result = algorithm.calculate(key, defaultPolicy)
  ...
  @Test fun `sliding precision, request mid-window only counts entries within rolling window`() = runBlocking {
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
      val result = algorithm.calculate(key, defaultPolicy)
  ...
  @Test fun `expired entries cleaned up, list does not grow unbounded across windows`() = runBlocking {
  ...
  @Test fun `high traffic key, many requests within window, memory grows linearly`() = runBlocking {
    // kept for consistency with redis tests. this was proven already by the sliding precision test
    val algorithm = RedisSlidingWindowLog(ScriptLoader(connection))
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
