...
class InMemorySlidingWindowLogTest {
  ...
  @Test
  fun `denied request returns retryAfter as time until oldest entry expires`() = runTest {
    val clock = FakeClock()
  ...
  @Test
  fun `requests outside window are pruned, counter resets after window passes`() = runTest {
    val clock = FakeClock()
    val algorithm = InMemorySlidingWindowLog(clock)
    val key = "test-key"
    repeat(defaultPolicy.limit.toInt()) { i ->
  ...
  @Test
  fun `expired entries cleaned up, list does not grow unbounded across windows`() = runTest {
  ...
  @Test
  fun `high traffic key, many requests within window, memory grows linearly`() = runTest {
    // kept for consistency with redis tests. this was proven already by the sliding precision test
    val clock = FakeClock()
    val algorithm = InMemorySlidingWindowLog(clock)
    val key = "test-key"
